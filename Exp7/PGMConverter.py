...
def convert_pgm_P5_batch(in_dir, out_dir, res_format):
    ...
    for file_name in file_list:
        ...
        if is_pgm_file(file_path):
            ...
            convert_pgm_by_PIL(file_path, file_out_path)
        # 若为目录，则新建结果文件目录，递归处理
...
if __name__ == '__main__':
    ...
    out_path=None
    isbatch=True
    in_dir='E:/3机器学习/Exp7/orl_faces/'
    out_dir='E:/3机器学习/Exp7/orl_faces(png)/'
    os.mkdir(out_dir)
    ...
    if in_path is not None and out_path is not None:
        ...
        # 批量转换
        cdir=list(range(1,41))
        odir=list(range(1,41))
        for i,x in enumerate(cdir):
            cdir[i]=in_dir+'s'+str(x)+'/'
        for i,x in enumerate(odir):
            odir[i]=out_dir+'s'+str(x)+'/'
            os.mkdir(odir[i])
        for i in range(0,40):
            convert_pgm_P5_batch(cdir[i], odir[i], res_format)
    else:
        print('请输入相应参数')
